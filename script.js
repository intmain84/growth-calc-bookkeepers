(function () {
  ...
  const Y_TICKS_BY_UNIT = {
    ...
    month: [
      300, 1000, 3000, 10000, 30000, 100000, 300000, 1000000, 3000000, 10000000,
      30000000,
    ],
    quarter: [
      3000, 10000, 30000, 100000, 300000, 1000000, 3000000, 10000000, 30000000,
      100000000,
    ],
    year: [
      3000, 10000, 30000, 100000, 300000, 1000000, 3000000, 10000000, 30000000,
  ...
  function safePow(base, exponent, cap) {
    let maxResult = isFiniteNumber(cap)
      ? clamp(cap, 1, MAX_FINITE_FLOW)
      : MAX_FINITE_FLOW;
  ...
  class GrowthCalculator {
    constructor(container, options) {
      ...
      this.state.units = isValidUnit(this.state.units)
        ? this.state.units
        : "year";
    ...
    _injectStyles() {
      ...
      style.textContent =
        ...
        "/* ========== RADIO BUTTON CUSTOMIZATION (Weekly/Monthly/Quarterly/Yearly) ========== */" +
    ...
    _build() {
      ...
      this.nodes.svg = svg;
      this.nodes.summaryBreakeven = summary.querySelector(
        '[data-key="breakeven"]',
      );
      ...
      this.nodes.inputGrossMargin = inputs.querySelector(
        '[data-key="grossMargin"]',
      );
    ...
    _setupSvgLayers() {
      ...
      this.chart = {
        ...
        yMin: flowToWeekly(
          Y_TICKS_BY_UNIT[this.state.units][0],
          this.state.units,
        ),
        yMax: flowToWeekly(
          Y_MAX_BY_UNIT[this.state.units] || Y_MAX_BY_UNIT.year,
          this.state.units,
        ),
        ticksY: [],
      };
    ...
    _stabilizeDrag(coords) {
      ...
      for (let pass = 0; pass < maxReprojectPasses; pass += 1) {
        ...
        this._updateYDomain();
        let nextYMaxLock =
          this.drag && this.drag.domain ? this.drag.domain.yMaxLock : NaN;
        let yMaxExpanded =
          isFiniteNumber(prevYMaxLock) &&
          isFiniteNumber(nextYMaxLock) &&
          nextYMaxLock > prevYMaxLock;
        if (!yMaxExpanded) {
          break;
        }
    ...
    _applyHeadroom(value, factor) {
      ...
      let scaled =
        safeValue >= maxBeforeScale ? projectionMax : safeValue * safeFactor;
    ...
    _filterYTicksByGap(ticks, minGap, reservedYPositions) {
      let sorted = ticks
        .filter(function (tick) {
          return (
            isFiniteNumber(tick) &&
            tick > 0 &&
            tick >= this.chart.yMin &&
            tick <= this.chart.yMax
          );
        }, this)
        .sort(function (a, b) {
          return a - b;
      ...
      for (let i = sorted.length - 1; i >= 0; i -= 1) {
        ...
        let nearReserved = (reservedYPositions || []).some(
          function (reservedY) {
      ...
      let fallback = [sorted[0], sorted[sorted.length - 1]].filter(
        function (tick, idx, arr) {
          return idx === 0 || Math.abs(tick - arr[idx - 1]) > 1e-9;
        },
      );
    ...
    _resolveDisplayDomain(
      ...
      if (visibleValues.length) {
        ...
        let hasMinVisibleDisplay =
          isFiniteNumber(minVisibleDisplay) && minVisibleDisplay > 0;
        yMinDisplayCandidate = hasMinVisibleDisplay
          ? minVisibleDisplay / 10
          : yMinDisplayCandidate;

        let yMaxCandidateWeekly = this._applyHeadroom(
          maxVisible,
          Y_HEADROOM_FACTOR,
        );
        let yMaxCandidateDisplay = flowFromWeekly(
          yMaxCandidateWeekly,
          displayUnit,
        );
        let hasMaxDisplay =
          isFiniteNumber(yMaxCandidateDisplay) && yMaxCandidateDisplay > 0;
        yMaxDisplayCandidate = hasMaxDisplay
          ? yMaxCandidateDisplay
          : yMaxDisplayCandidate;
      }

      let yMinDisplay = Math.max(displayFloor, yMinDisplayCandidate);
      let yMaxDisplay = yMaxDisplayCandidate;
      let invalidDomain =
        !isFiniteNumber(yMinDisplay) ||
        !isFiniteNumber(yMaxDisplay) ||
        yMinDisplay <= 0;
      if (invalidDomain) {
    ...
    _applyChartDomain(displayUnit, yMinDisplay, yMaxDisplay) {
      this.chart.yMin = this._clampForProjection(
        flowToWeekly(yMinDisplay, displayUnit),
      );
      this.chart.yMax = this._clampForProjection(
        flowToWeekly(yMaxDisplay, displayUnit),
      );
    ...
    _buildDisplayTicks(baselineDisplayTicks, yMinDisplay, yMaxDisplay) {
      ...
      let ticksDisplay = hasBaselineTicks
        ? baselineDisplayTicks.slice()
        : createOneThreeTicks(yMinDisplay, yMaxDisplay, targetCount);
      let baselineMax = hasBaselineTicks
        ? baselineDisplayTicks[baselineDisplayTicks.length - 1]
        : NaN;
      let shouldExtendBaseline =
        hasBaselineTicks &&
        isFiniteNumber(baselineMax) &&
        yMaxDisplay > baselineMax;
      if (shouldExtendBaseline) {
        let extension = createOneThreeTicks(
          Math.max(yMinDisplay, baselineMax),
          yMaxDisplay,
          targetCount * 2,
        );
        ticksDisplay = ticksDisplay.concat(
          extension.filter(function (tick) {
      ...
      ticksDisplay = ticksDisplay
        .filter(function (tick) {
          return (
            isFiniteNumber(tick) &&
            tick >= yMinDisplay * 0.99 &&
            tick <= yMaxDisplay * 1.01
          );
        })
        .sort(function (a, b) {
          return a - b;
    ...
    _setTicksFromDisplayDomain(
      displayUnit,
      baselineDisplayTicks,
      yMinDisplay,
      yMaxDisplay,
    ) {
      let ticksDisplay = this._buildDisplayTicks(
        baselineDisplayTicks,
        yMinDisplay,
        yMaxDisplay,
      );
      ...
      if (!this.chart.ticksY.length) {
        let safeMinDisplay =
          isFiniteNumber(yMinDisplay) && yMinDisplay > 0 ? yMinDisplay : 1;
        let safeMaxDisplay =
          isFiniteNumber(yMaxDisplay) && yMaxDisplay > safeMinDisplay
            ? yMaxDisplay
            : safeMinDisplay * 10;
        let midDisplay = Math.sqrt(safeMinDisplay * safeMaxDisplay);
        this.chart.ticksY = [
          flowToWeekly(
            clamp(midDisplay, safeMinDisplay, safeMaxDisplay),
            displayUnit,
          ),
        ];
      }
    ...
    _updateYDomainDuringDrag(dragDomain, displayUnit, baselineDisplayTicks) {
      ...
      let neededYMax = visibleValues.length
        ? this._applyHeadroom(
            Math.max.apply(null, visibleValues),
            Y_HEADROOM_FACTOR,
          )
        : NaN;
      ...
      let expandedYMax =
        isFiniteNumber(neededYMax) && neededYMax > currentYMaxLock
          ? this._snapUpToOneThreeTick(neededYMax)
          : currentYMaxLock;
      let nextYMaxLock =
        expandedYMax > currentYMinLock
          ? expandedYMax
          : this._clampForProjection(currentYMinLock * 1.2);
      let yMaxExpanded = nextYMaxLock > currentYMaxLock;
      let hasLockedTicks =
        Array.isArray(dragDomain.ticksYLock) &&
        dragDomain.ticksYLock.length > 0;
    ...
    _revenueAt(tYearsFromStart) {
      ...
      let maxMultiplier =
        MAX_FINITE_FLOW /
        Math.max(this.state.weeklyRevenue0, MIN_WEEKLY_LOG_FLOOR);
    ...
    _computeMetrics() {
      ...
      if (canSolveBreakeven) {
        ...
        let hasSolvedBreakeven =
          isFiniteNumber(solvedWeeks) && solvedWeeks >= 0;
        breakevenYears = hasSolvedBreakeven
          ? solvedWeeks / WEEKS_PER_YEAR
          : breakevenYears;
      }
      ...
      if (canSolveBillion) {
        let solvedBillionWeeks =
          Math.log(weeklyBillionTarget / rev0) / Math.log(1 + growth);
        let hasSolvedBillion =
          isFiniteNumber(solvedBillionWeeks) && solvedBillionWeeks >= 0;
        billionYears = hasSolvedBillion
          ? solvedBillionWeeks / WEEKS_PER_YEAR
          : billionYears;
      }
    ...
    _updateInputs() {
      ...
      this.nodes.inputGrossMargin.value = formatInputPercent(
        this.state.grossMargin,
      );
      ...
      let displayGrowth = growthFromWeekly(
        this.state.weeklyGrowthRate,
        this.state.units,
      );
    ...
    _updateYDomain() {
      ...
      let baselineDisplayTicks =
        Y_TICKS_BY_UNIT[displayUnit] || Y_TICKS_BY_UNIT.year || [];

      if (this._hasValidDragDomain(dragDomain)) {
        this._updateYDomainDuringDrag(
          dragDomain,
          displayUnit,
          baselineDisplayTicks,
        );
      ...
      let fallbackYMaxDisplay =
        (Y_MAX_BY_UNIT[displayUnit] || Y_MAX_BY_UNIT.year) * Y_HEADROOM_FACTOR;
      let visibleValues = this._visibleValuesForYDomain();
      let displayDomain = this._resolveDisplayDomain(
        displayUnit,
        displayFloor,
        fallbackYMaxDisplay,
        visibleValues,
      );
      let normalizedDomain = this._applyChartDomain(
        displayUnit,
        displayDomain.yMinDisplay,
        displayDomain.yMaxDisplay,
      );
      this._setTicksFromDisplayDomain(
        displayUnit,
        baselineDisplayTicks,
    ...
    _xToTime(x) {
      ...
      let clamped = clamp(
        ...
        this.chart.width - this.chart.paddingRight,
      );
    ...
    _yFromValue(value) {
      ...
      let safeValue = isFiniteNumber(projectionValue)
    ...
    _yToValue(y) {
      ...
      let ratio =
        (this.chart.height - this.chart.paddingBottom - clamped) / plotHeight;
      let lnMin = Math.log(this.chart.yMin);
      let lnMax = Math.log(this.chart.yMax);
    ...
    _linePath(fn, segments) {
      ...
      let sampleSegments = clamp(
        Math.round(
          isFiniteNumber(segments) ? segments : this._totalLineSegments(),
        ),
        2,
      ...
      for (let i = 0; i <= sampleSegments; i += 1) {
        ...
        points.push(
          this._xFromTime(t) + "," + this._yFromValue(fn.call(this, t)),
        );
      }
    ...
    _lineSegmentPath(fn) {
      ...
      let tEnd = this.chart.tMax - this.chart.tMin;
      let startPoint =
        this._xFromTime(tStart) + "," + this._yFromValue(fn.call(this, tStart));
      let endPoint =
        this._xFromTime(tEnd) + "," + this._yFromValue(fn.call(this, tEnd));
    ...
    _layoutLineLabelsY(candidates, minY, maxY, minGap) {
      ...
      let sorted = candidates
        ...
        .map(function (candidate) {
          let idealY = clamp(
            candidate.targetY + (candidate.dy || 0),
            minY,
            maxY,
          );
      ...
      let effectiveGap =
        sorted.length < 2 ? 0 : Math.min(minGap, range / (sorted.length - 1));
      ...
      return [
        {
          text: "Revenue",
          color: COLORS.revenue,
          targetY: this._yFromValue(this._revenueAt(tLabel)),
          dy: -6,
        },
        {
          text: "Total expenses",
          color: COLORS.total,
          targetY: this._yFromValue(this._totalAt(tLabel)),
          dy: 8,
        },
        {
          text: "Variable expenses",
          color: COLORS.variable,
          targetY: this._yFromValue(this._variableAt(tLabel)),
          dy: 10,
        },
        {
          text: "Fixed expenses",
    ...
    _handleLabelText(handle) {
      ...
      if (handle === "fixed") {
        let fixedDisplayValue = flowFromWeekly(
          this.state.weeklyFixedExpenses,
          this.state.units,
        );
    ...
    _renderActiveHandleLabel(group, activeHandle, handlePoints, bounds) {
      let isSupportedHandle =
        activeHandle === "growth" ||
        activeHandle === "variable" ||
        activeHandle === "fixed";
    ...
    _draw(metrics) {
      ...
      let yearLabelStep = Math.max(
        1,
        Math.ceil(MIN_X_YEAR_LABEL_GAP / Math.max(1, yearSpacing)),
      );
      ...
      for (let year = this.chart.tMin; year <= this.chart.tMax; year += 1) {
        ...
        let shouldShowYearLabel =
          t % yearLabelStep === 0 || year === this.chart.tMax;
      ...
      let inlineLabels = this._layoutInlineLineLabels(
        inlineLabelCandidates,
        labelMinY,
        labelMaxY,
        INLINE_LINE_LABEL_MIN_GAP,
      );
      let inlineLabelX = this._xFromTime(inlineLabelT) + 8;
      this._renderInlineLineLabels(gLabels, inlineLabels, inlineLabelX);

      let rightLabelCandidates = this._rightLineLabelCandidates(tEnd).filter(
        function (candidate) {
          let isSuppressed =
      ...
      let outsideLabels = this._layoutRightLineLabels(
        outsideCandidates,
        labelMinY,
        labelMaxY,
      ...
      let insideLabels = this._layoutRightLineLabels(
        insideCandidates,
        labelMinY,
        labelMaxY,
        RIGHT_LINE_LABEL_MIN_GAP,
        ...
        item.x =
          plotRightX -
          (HANDLE_RECT_VISUAL_WIDTH / 2 +
            RIGHT_LABEL_INSIDE_PADDING_FROM_HANDLE);
        item.anchor = "end";
        return item;
      });
      this._renderRightLineLabels(
        gLabels,
        outsideLabels.concat(insideLabels),
        plotRightX,
      );
      ...
      addHandleRect(
        "fixed",
        handlePoints.fixed.x,
        handlePoints.fixed.y,
        COLORS.fixed,
      );
      addHandleRect(
        "variable",
        handlePoints.variable.x,
        handlePoints.variable.y,
        COLORS.variable,
      );
      addHandleCircle(
        "growth",
        handlePoints.growth.x,
        handlePoints.growth.y,
        COLORS.revenue,
      );
      ...
      let isExpenseHandleActive =
        activeHandle === "fixed" || activeHandle === "variable";
    ...
    render(options) {
      ...
      if (!opts.skipKpis && metrics !== null) {
        this.nodes.summaryBreakeven.textContent = this._formatTime(
          metrics.breakevenYears,
        );
        this.nodes.summaryBillion.textContent = this._formatTime(
          metrics.billionYears,
        );
      }
    }
  }
